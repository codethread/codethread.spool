(ns codethread.spools.ralph
  ...
  `workflow/context` conforms to `:codethread.spools.ralph/ralph-context`."
  ...
  (workflow/workflow
   ...
   (workflow/step :finish-feature
                  ...
                  :attributes {"workflow/action-ref" "ralph.finish-feature"
                               "workflow/instruction"
                               (format-alpha/reflow
                                "|After the validated slice is handed to the consumer's own
                                 |landing policy, re-read this run's `workflow/context` and use
                                 |its validated `ralph/card` value. Verify that exact feature card
                                 |is closed with `kanban/outcome=done`. If it is still active,
                                 |finish it with `strand kanban finish <ralph/card> --outcome done`;
                                 |leave a concise summary note on the card's doing-task. If the
                                 |context is missing or does not match the feature card, stop and
                                 |record the mismatch instead of guessing.")})
