...
func Prompt(epicID, epicTitle string, iteration int, fullAuth bool) string {
	...
	b.WriteString("--- ralph harness (Codethread Ralph spool, iteration ")
	...
	b.WriteString("Close the epic only through the workflow’s checked completion gate after every feature has outcome done.\n")
	b.WriteString("An empty runnable frontier with unfinished children means leave the epic open and report the landing wait or blocker with RALPH-STOP.\n")
	...
	b.WriteString("Use it for hard blockers or a landing wait with no runnable work; it halts the loop immediately.\n")
	if fullAuth {
		b.WriteString("\n")
