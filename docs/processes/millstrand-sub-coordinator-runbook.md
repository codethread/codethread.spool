...
Never stop or restart Mill. Never restart or replace a running Weaver without explicit user sign-off. Never use deprecated `agent-harness.spool`; maintained harness work belongs in `harnesses.spool`. Stop only an identified run or PID, never a broad process-name kill. Never edit or push `main`. Use disposable, explicit workspaces for workspace-backed tests, never the shared world.
