...
The shared [Codethread bootstrap](/Users/ct/dev/projects/codethread.spool/spools/config/src/ct/spools/codethread/bootstrap.clj:10) activates the base Workflow engine. Each standard workspace then selects Millhouse's provider module explicitly; see [Codethread startup](/Users/ct/dev/projects/codethread.spool/.millstrand/init.clj:12) and the [owner selector](/Users/ct/dev/projects/millhouse.spool/spools/workflow/src/millhouse/spools/workflow/spool.clj:25). This selector publishes the surviving publisher checklist.
...
The review rejected a false positive in Terra's first report. Its fixture used unsuffixed rule and executor names. Chime actually generates `<name>-rule`, and Workflow generates `<name>-stalled?`; the hooks model those names correctly. Changing only the four use arguments eliminated all four reported errors. The corresponding authoring plans are [Chime](/Users/ct/dev/projects/millhouse.spool/spools/chime/src/millhouse/spools/chime.clj:86) and [Workflow](/Users/ct/dev/projects/millhouse.spool/spools/workflow/src/millhouse/spools/workflow.clj:1015). There is no demonstrated hook defect to turn into a replacement card.
...
The current [Codethread quality script](/Users/ct/dev/projects/codethread.spool/scripts/quality.sh:1) does not refresh imported Kondo configs. Treat import refresh as an explicit setup/dependency-update responsibility; automate it only if that is a recurring maintenance problem.
